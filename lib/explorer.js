...
module.exports = {
  ...
  get_supply: function(cb) {
    ...
    } else if (settings.sync.supply == 'HEAVY') {
      // attempt to get the max supply from the getmaxsupply or similar api cmd that returns the current money supply as a single positive decimal value
      var cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getmaxsupply);
